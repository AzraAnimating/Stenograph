...
use crate::{generate_response, storage::database, structs::{files::{DataType, GetFile, GetPDF}, state::AppState}};
...
pub async fn submit_pdf(State(state): State<AppState>, mut multipart: Multipart) -> impl IntoResponse {
    ...
    let id = match database::add_file(&state.database, file_name.expect("Failed to get validated Value"), DataType::PDF).await {
        Ok(id) => id,
        Err(err) => {
...
pub async fn retrieve_pdf(Json(file_id): Json<GetFile>) -> impl IntoResponse {
...
pub async fn submit_text(State(state): State<AppState>, mut multipart: Multipart) -> impl IntoResponse {
    ...
    while let Some(field) = match multipart.next_field().await {
        ...
        if name.eq("upload") {
            data = match field.bytes().await {
                Ok(upload_bytes) => Some(upload_bytes),
                Err(err) => {
                    return err.into_response();
                },
            };
    ...
    let id = match database::add_file(&state.database, file_name.expect("Failed to get validated Value"), DataType::HTML).await {
        Ok(id) => id,
        Err(err) => {
            return generate_response!(StatusCode::INTERNAL_SERVER_ERROR, err);
        },
    ...
    let mut file = match File::create(format!("files/{}.html", id)).await {
        Ok(file) => file,
        Err(err) => {
            return generate_response!(StatusCode::INTERNAL_SERVER_ERROR, err.to_string());
        },
...
pub async fn retrieve_text(Json(file_id): Json<GetFile>) -> impl IntoResponse {
    let file = match File::open(format!("files/{}.hmtl", &file_id.uuid)).await {
        Ok(file) => file,
        Err(err) => {
            return generate_response!(StatusCode::NOT_FOUND, "No such File");
        },
